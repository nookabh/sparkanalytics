...
class DraftContractWithPricingVol(optyAcc_Df: DataFrame, base_df: DataFrame) extends CXTransformationBase {
  /* @author Bharath
   	Metric : Draft With Pricing Volume  Metric
 	*/
  try {
    val head = "\"SFDCOppId\",\"EventName\",\"EventType\",\"Count\"" + System.lineSeparator
    this.writeFile(calculateVPApprovalVolume, "DRAFT_WITH_PRICING_VOL", head)
    def calculateVPApprovalVolume(): DataFrame = {
      val based_result = base_df.filter(base_df("MILESTONE_ID") === "10601013").groupBy("short_oppty_id").count()
      ...
      var final_df = based_result.join(optyAcc, optyAcc("SFDC_OPPORTUNITY_ID__C") === based_result("short_oppty_id")).drop(col("SFDC_OPPORTUNITY_ID__C")).withColumn("EventType", lit("ROLLING"))
        .withColumn("EventName", lit("DRAFT_WITH_PRICING_VOL"))
