  ...
  //Logger.getLogger("akka").setLevel(Level.OFF)
	  ...
	  if (isServer == true) {
	      ...
	      val jobName = "PPAWEEK"
				...
				var csvInPath_quote = "C:/SparkAnalytics/CX_PQ_089012017.csv"
				var csvInPath_opp = "C:/SparkAnalytics/cx_opptyhist_oppty_sfdc.txt"
...
//				.option("delimiter", "|")
				...
				.drop(base_df.col("Quote_version"))
				.drop(base_df.col("linked_quote_create_date"))
				.drop(base_df.col("base_quote_id"))
				.drop(base_df.col("Oppty_create_Date"))
				.drop(base_df.col("Long_oppty_id"))
				.drop(base_df.col("MILESTONE_ID"))
				.dropDuplicates()
								
				base_quote_df.show()
				base_quote_df = base_quote_df.filter(base_quote_df.col("milestone_date").isNotNull)
				                             .filter("milestone_date != ''")
				println("not null")
				base_quote_df.show()				
				...
				var base_quote_min_df = base_quote_df.groupBy("short_oppty_id").agg(min("milestone_sec"))
				                                     .withColumnRenamed("short_oppty_id" , "SFDC_OPP")
	  	  base_quote_df = base_quote_min_df.join(base_quote_df,
		                    base_quote_df("short_oppty_id") === base_quote_min_df("SFDC_OPP") && 
		                    base_quote_df("milestone_sec") === base_quote_min_df("MIN(milestone_sec)"))
		                    
	  	  base_quote_df = base_quote_df.drop(base_quote_df.col("MIN(milestone_sec)"))
	  	                               .drop(base_quote_df.col("SFDC_OPP"))
	      ...
	      var run_date_sec = date_format.parse(run_date).getTime()
				base_df_opp = base_df_opp.drop(base_df_opp.col("OPPTY_Hist_ID"))
				                         .drop(base_df_opp.col("SYSTEMMODSTAMP"))	
				                         .withColumn("timestampsec", udfToTime(base_df_opp("CreatedDate")))
				                         
				base_df_opp = base_df_opp.filter(base_df_opp.col("timestampsec") >= run_date_sec)
			                           .filter("StageName != '5 Closed Lost'")
			                           .filter("StageName != '5 Closed Disqualified'")
			                           .filter("StageName != '0 Identify'")
			                           .filter("StageName != '1 Qualify'")
			    
			  var opp_max_validity_df = base_df_opp.groupBy("SFDC_OPPORTUNITY_ID__C").agg(max("timestampsec"))
				var opp_min_validity_df = base_df_opp.groupBy("SFDC_OPPORTUNITY_ID__C").agg(min("timestampsec"))
				var opp_validity_df = opp_max_validity_df.join(opp_min_validity_df, "SFDC_OPPORTUNITY_ID__C")
				opp_validity_df = opp_validity_df.withColumnRenamed("MIN(timestampsec)" , "valid_start_sec")
				                                 .withColumnRenamed("MAX(timestampsec)" , "valid_end_sec")
				base_quote_df = base_quote_df.join(opp_validity_df, base_quote_df.col("short_oppty_id") === opp_validity_df("SFDC_OPPORTUNITY_ID__C"))
				                                 
				base_quote_df = base_quote_df.withColumn("is_valid", when(base_quote_df("milestone_sec") >= base_quote_df("valid_start_sec") &&
				    base_quote_df("milestone_sec") <= base_quote_df("valid_end_sec"), lit("1")).otherwise(lit("0")) )
				          
				 base_quote_df = base_quote_df.filter("is_valid = '1'")
				 base_quote_df.show()
				...
				base_quote_df.show()
				var base_quote_min_df = base_quote_df.groupBy("short_oppty_id").agg(min("milestone_sec"))
				base_quote_min_df = base_quote_min_df.withColumnRenamed("short_oppty_id" , "SFDC_OPP")
	  	  base_quote_df = base_quote_min_df.join(
	  	      base_quote_df,
		      base_quote_df("short_oppty_id") === base_quote_min_df("SFDC_OPP")
		      && base_quote_df("milestone_sec") === base_quote_min_df("MIN(milestone_sec)")
	  	      )
        ...
        println(cal_today.getTimeInMillis())
				...
				while(cal_rundate.compareTo(cal_today) < 0 ) {  
				  ...
				  var week_start_sec = run_date_sec - 604800000
				  if (isServer == true) {
				   ...
				   println("Run date in loop" + DateTimeFormat.forPattern("YYYY_MM_dd").print(run_date_sec))				  
				   println(run_date_sec)

		    var base_quote_period_df =  base_quote_df.filter(base_quote_df.col("MIN(milestone_sec)") <= run_date_sec )
		    base_quote_period_df = base_quote_period_df.filter(base_quote_period_df.col("MIN(milestone_sec)") >= week_start_sec)				   
				
		    var opp_hist_period_df = base_df_opp.filter(base_df_opp.col("timestampsec") <= run_date_sec)
		    opp_hist_period_df = opp_hist_period_df.join(
		        base_quote_period_df,
		        base_quote_period_df("short_oppty_id") === opp_hist_period_df("SFDC_OPPORTUNITY_ID__C"))
		    opp_hist_period_df = opp_hist_period_df.drop(opp_hist_period_df.col("PP_Quote_Type"))    
				opp_hist_period_df.show()
				println("opp_hist_period_df count" + opp_hist_period_df.count() )
		  ...
		  println("current_stage_df" + current_stage_df.count() )
		  

		  base_quote_period_df = base_quote_period_df.join(current_stage_df, base_quote_period_df("short_oppty_id") === current_stage_df("SFDC_OPPORTUNITY_ID__C"))
		  
		  println("base_quote_period_df" + base_quote_period_df.count() )
			...
			println("current_stage_df valid stages" + current_stage_df.count() )
			//current_stage_df = current_stage_df.filter("StageName != '5 Closed Lost'")
		  ...
		  println("current_stage_df")
			...
			base_quote_period_df = base_quote_period_df.withColumn("is_basequote", when(base_quote_period_df("PP_Quote_Type") === "Base Quote", lit("1")).otherwise(lit("0")) )
			.select("short_oppty_id", "is_basequote")
			println("base_quote_period_df")
			base_quote_period_df.show()
