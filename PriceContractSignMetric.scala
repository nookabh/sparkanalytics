package com.workflow.cx
 ...
 /* @author bnookala
 *  NVDW-492
 * The time between when the final price is validated to Contract Signature (CDCV Accept Milestone)	PQ	"10301050 - Price Quote Validated
...
class PriceContractSignMetric(base_df: DataFrame) extends CXTransformationBase  {
  try {
  val head = "\"SFDCOppId\",\"EventName\",\"EventType\",\"EventStartDate\",\"EventCompletionDate\",\"Count\"" + System.lineSeparator
  this.writeFile(calculatePriceContractSignMetric, "PRICE_CONTRACT_SIGN",head)
  def calculatePriceContractSignMetric():DataFrame={
   val findDiffUDF = udf(UtilityClass.findDiff _)
      ...
      val created_df = parsed_df.filter(base_df("MILESTONE_ID") === "10301050") //get the min
      val draft_df = parsed_df.filter(base_df("MILESTONE_ID") === "10601020") //get the latest
      ...
      val parsedEventCompletionDate = parsedeventstartdate.withColumn("EventCompletionDate", parser(result_df("EndDate")))
                                      ...
                                      .withColumn("EventName", lit("PRICE_CONTRACT_SIGN"))
