...
object PricingNotEquals extends CXTransformationBase {
...
//    System.setProperty("hadoop.home.dir", "/etc/hadoop/conf");
    ...
    def getpriceNotEquals(): DataFrame = {
      csvInPath = "C://Users//nookabh//Desktop//json//pricefiles//price//priceNotEquals.csv"
     ...
     def getpriceEquals(): DataFrame = {
      ...
      var rawTextRDD = sc.textFile(csvInPath)
      rawTextRDD = rawTextRDD.mapPartitionsWithIndex { (idx, iter) => if (idx == 0) iter.drop(1) else iter }
      val quoteSchemaFields = "cntry_code,PR4,postal_number,ACCOUNT_NASP_NAME__C,count(SITE_ID),min(Total_Price),avg(Total_Price),max(Total_Price)".split(",")
        .map(fieldName => StructField(fieldName, StringType, nullable = true))
      val quoteSchema = StructType(quoteSchemaFields)
      val rowRDD = rawTextRDD.map(_.split(",")).map(p => {
        if (p.length == 8) {
    ...
    pricenotequals.show()
    pricenotequals.filter(pricenotequals("PR4") === "Private IP").show()
